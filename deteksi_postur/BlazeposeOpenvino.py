...
from math import atan2,atan,degrees,acos,sqrt
    ...
    def lm_render(self, frame, region):
        ...
        if region.lm_score > self.lm_score_threshold:
            ...
            if self.show_landmarks:
                ...
                x2 = int((region.landmarks_padded[23][0]+region.landmarks_padded[24][0])/2)
                y2 = int((region.landmarks_padded[23][1]+region.landmarks_padded[24][1])/2)
                x3 = int((lines[0][7][0]+lines[0][6][0])/2)
                y3 = int((lines[0][7][1]+lines[0][6][1])/2)
                distance = (distanceCalculate(region.landmarks_padded[25],region.landmarks_padded[26]))
                # print(region.landmarks_padded[23])
                # print(region.landmarks_padded[24])
                result = AngleBtw2Points(lines[0][7],lines[0][6])
                if result < 0:
                    result+=180
                elif result > 0 :
                    result-=180
                
                cv2.putText(frame,f"Derajat Antara 2 Bahu {result:.2f}",(x3-90, y3+40),cv2.FONT_HERSHEY_PLAIN, 1, (0,0,255), 2)
                cv2.putText(frame,f"Jarak antara Lutut {distance:.2f}",(region.landmarks_padded[25,0]-120, region.landmarks_padded[25,1]+30),cv2.FONT_HERSHEY_PLAIN, 1, (0,0,255), 2)
                ...
                for i,x_y in enumerate(region.landmarks_padded[:self.nb_lms-2,:2]):
                    if i > 10:
                        ...
                        color = (0,0,255)
                    cv2.circle(frame, (x2, y2), 4, color)
                    cv2.circle(frame,(x3,y3),4, color, -11)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--input', type=str, default='0', help="Path to video or image file to use as input (default=%(default)s)")
    parser.add_argument('-g', '--gesture', action="store_true", help="enable gesture recognition")
    parser.add_argument("--pd_xml", type=str, help="Path to an .xml file for pose detection model")
    parser.add_argument("--pd_device", default='CPU', type=str,help="Target device for the pose detection model (default=%(default)s)")  
    parser.add_argument("--lm_xml", type=str, help="Path to an .xml file for landmark model")
    parser.add_argument("--lm_version", type=str, choices=['full', 'lite', 'heavy'], default="full",help="Version of the landmark model (default=%(default)s)")
    parser.add_argument("--lm_device", default='CPU', type=str, help="Target device for the landmark regression model (default=%(default)s)")
    parser.add_argument('--min_tracking_conf', type=float, default=0.5, help="Minimum confidence value ([0.0, 1.0]) from the landmark-tracking model for the pose landmarks to be considered tracked successfully,"+
                        " or otherwise person detection will be invoked automatically on the next input image. (default=%(default)s)")                    
    parser.add_argument('-c', '--crop', action="store_true",  help="Center crop frames to a square shape before feeding pose detection model")
    parser.add_argument('--no_smoothing', action="store_true", help="Disable smoothing filter")
    parser.add_argument('--filter_window_size', type=int, default=5, help="Smoothing filter window size. Higher value adds to lag and to stability (default=%(default)i)")                    
    parser.add_argument('--filter_velocity_scale', type=float, default=10, help="Smoothing filter velocity scale. Lower value adds to lag and to stability (default=%(default)s)")                    
    parser.add_argument('-3', '--show_3d', action="store_true",  help="Display skeleton in 3d in a separate window (valid only for full body landmark model)")
    parser.add_argument("-o","--output", help="Path to output video file")
    parser.add_argument('--multi_detection', action="store_true",   help="Force multiple person detection (at your own risk, the original Mediapipe implementation is designed for one person tracking)")
    parser.add_argument('--force_detection', action="store_true",  help="Force person detection on every frame (never use landmarks from previous frame to determine ROI)")
